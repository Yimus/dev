...
public class LoggingAspect {
    ...
    @Before("execution(* com.example.order.controller.*.*(..))")
    ...
    @After("execution(* com.example.order.controller.*.*(..))")
    public void logAfterController(JoinPoint joinPoint) {
        LOGGER.info("-- {} end --", joinPoint.getSignature().getName());
    }

    @AfterThrowing(pointcut = "execution(* com.example.order.controller.*.*(..))", throwing = "ex")
